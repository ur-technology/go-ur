...
// Package miner implements UR block creation and mining.
package miner
...
type Miner struct {
	...
	canStart    int32 // can start indicates whur we can start the mining operation
	shouldStart int32 // should start indicates whur we should start after sync
}

func New(ur core.Backend, mux *event.TypeMux, pow pow.PoW) *Miner {
	miner := &Miner{ur: ur, mux: mux, pow: pow, worker: newWorker(common.Address{}, ur), canStart: 1}
	go miner.update()
...
func (self *Miner) SetURbase(addr common.Address) {
	self.coinbase = addr
	self.worker.setURbase(addr)
}

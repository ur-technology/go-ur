...
type fetchRequest struct {
	...
	Hashes  map[common.Hash]int // [ur/61] Requested hashes with their insertion index (priority)
	Headers []*types.Header     // [ur/62] Requested headers, sorted by request order
...
type queue struct {
	...
	hashPool    map[common.Hash]int // [ur/61] Pending hashes, mapping to their insertion index (priority)
	hashQueue   *prque.Prque        // [ur/61] Priority queue of the block hashes to fetch
	hashCounter int                 // [ur/61] Counter indexing the added hashes to ensure retrieval order

	headerHead common.Hash // [ur/62] Hash of the last queued header to verify order

	blockTaskPool  map[common.Hash]*types.Header // [ur/62] Pending block (body) retrieval tasks, mapping hashes to headers
	blockTaskQueue *prque.Prque                  // [ur/62] Priority queue of the headers to fetch the blocks (bodies) for
	blockPendPool  map[string]*fetchRequest      // [ur/62] Currently pending block (body) retrieval operations
	blockDonePool  map[common.Hash]struct{}      // [ur/62] Set of the completed block (body) fetches

	receiptTaskPool  map[common.Hash]*types.Header // [ur/63] Pending receipt retrieval tasks, mapping hashes to headers
	receiptTaskQueue *prque.Prque                  // [ur/63] Priority queue of the headers to fetch the receipts for
	receiptPendPool  map[string]*fetchRequest      // [ur/63] Currently pending receipt retrieval operations
	receiptDonePool  map[common.Hash]struct{}      // [ur/63] Set of the completed receipt fetches

	stateTaskIndex int                      // [ur/63] Counter indexing the added hashes to ensure prioritised retrieval order
	stateTaskPool  map[common.Hash]int      // [ur/63] Pending node data retrieval tasks, mapping to their priority
	stateTaskQueue *prque.Prque             // [ur/63] Priority queue of the hashes to fetch the node data for
	statePendPool  map[string]*fetchRequest // [ur/63] Currently pending node data retrieval operations

	stateDatabase   urdb.Database   // [ur/63] Trie database to populate during state reassembly
	stateScheduler  *state.StateSync // [ur/63] State trie synchronisation scheduler and integrator
	stateProcessors int32            // [ur/63] Number of currently running state processors
	stateSchedLock  sync.RWMutex     // [ur/63] Lock serialising access to the state scheduler
...
func newQueue(stateDb urdb.Database) *queue {
	lock := new(sync.Mutex)
	return &queue{
...
// InFlightBlocks retrieves whur there are block fetch requests currently in
// flight.
...
// InFlightReceipts retrieves whur there are receipt fetch requests currently
// in flight.
...
// InFlightNodeData retrieves whur there are node data entry fetch requests
// currently in flight.
...
func (q *queue) reserveHashes(p *peer, count int, taskQueue *prque.Prque, taskGen func(int), pendPool map[string]*fetchRequest, maxPending int) *fetchRequest {
	// Short circuit if the peer's already downloading somuring (sanity check to
	// not corrupt state)
...
// returns a flag whur empty blocks were queued requiring processing.
...
// also returns a flag whur empty receipts were queued requiring importing.
...
func (q *queue) reserveHeaders(p *peer, count int, taskPool map[common.Hash]*types.Header, taskQueue *prque.Prque,
	...
	// downloading somuring (sanity check not to corrupt state)
	if taskQueue.Empty() {

...
// Package xur is the interface to all UR functionality.
package xur
...
import (
	...
	"github.com/ur/go-ur/ur/filters"
...
type XEth struct {
	...
	gpo           *ur.GasPriceOracle
	state         *State
	whisper       *Whisper
	filterManager *filters.FilterSystem
}

func NewTest(ur *ur.UR, frontend Frontend) *XEth {
	return &XEth{backend: ur, frontend: frontend}
}

// New creates an XEth that uses the given frontend.
// If a nil Frontend is provided, a default frontend which
// confirms all transactions will be used.
func New(urk *ur.UR, frontend Frontend) *XEth {
	xur := &XEth{
		backend:          urk,
		frontend:         frontend,
		quit:             make(chan struct{}),
		filterManager:    filters.NewFilterSystem(urk.EventMux()),
		logQueue:         make(map[int]*logQueue),
		blockQueue:       make(map[int]*hashQueue),
		transactionQueue: make(map[int]*hashQueue),
		messages:         make(map[int]*whisperFilter),
		agent:            miner.NewRemoteAgent(),
		gpo:              ur.NewGasPriceOracle(urk),
	}
	if urk.Whisper() != nil {
		xur.whisper = NewWhisper(urk.Whisper())
	}
	urk.Miner().Register(xur.agent)
	if frontend == nil {
		xur.frontend = dummyFrontend{}
	}
	state, _ := xur.backend.BlockChain().State()
	xur.state = NewState(xur, state)
	go xur.start()
	return xur
}
...
func (self *XEth) WithState(statedb *state.StateDB) *XEth {
	...
	xur.state = NewState(xur, statedb)
	return xur
}
...
func (p *XEth) NewWhisperFilter(to, from string, topics [][]string) int {
	...
	// Callback to delegate core whisper messages to this xur filter
	...
	// Initialize the core whisper filter and wrap into xur

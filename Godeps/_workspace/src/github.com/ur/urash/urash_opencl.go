...
/*
  ...
  1. https://github.com/ur/wiki/wiki/Ethash
  2. https://github.com/ur/cpp-ur/blob/develop/liburash-cl/urash_cl_miner.cpp
...
type OpenCLMiner struct {
	...
	urash *Ethash // Ethash full DAG & cache in host mem
...
func PrintDevices() {
	...
	if len(found) == 0 {
		...
		fmt.Printf("Found %v devices. Benchmark first GPU:       gur gpubench 0\n", len(found))
		fmt.Printf("Mine using all GPUs:                        gur --minegpu %v\n", idsFormat)
	}
}
...
func InitCL(blockNum uint64, c *OpenCLMiner) error {
	platforms, err := cl.GetPlatforms()
	if err != nil {
		return fmt.Errorf("Plaform error: %v\nCheck your OpenCL installation and then run gur gpuinfo", err)
	}
	...
	for _, p := range platforms {
		...
		if err != nil {
			return fmt.Errorf("Devices error: %v\nCheck your GPU drivers and then run gur gpuinfo", err)
		}
		for _, d := range ds {
	...
	c.urash = pow
	dagSize := uint64(C.urash_get_datasize(C.uint64_t(blockNum)))
	c.dagSize = dagSize

	for _, id := range c.deviceIds {
		if id > len(devices)-1 {
			return fmt.Errorf("Device id not found. See available device ids with: gur gpuinfo")
		} else {
...
func initCLDevice(deviceId int, device *cl.Device, c *OpenCLMiner) error {
	...
	kvs["DAG_SIZE"] = strconv.FormatUint(c.dagSize/urashMixBytesLen, 10)
	kvs["ACCESSES"] = strconv.FormatUint(urashAccesses, 10)
	...
	*/
	...
	searchKernelName = "urash_search"
	hashKernelName = "urash_hash"
	...
	dagPtr := unsafe.Pointer(c.urash.Full.current.ptr.data)
...
func (c *OpenCLMiner) Search(block pow.Block, stop <-chan struct{}, index int) (uint64, []byte) {
	c.mu.Lock()
	newDagSize := uint64(C.urash_get_datasize(C.uint64_t(block.NumberU64())))
		...
		if len(pending) == searchBufSize {
			...
			for i := uint32(0); i < nfound; i++ {
				...
				if checkNonce != 0 {
					...
					ret := C.urash_light_compute_internal(c.urash.Light.current.ptr, ds, hashToH256(headerHash), cn)
...
func (c *OpenCLMiner) Verify(block pow.Block) bool {
	return c.urash.Light.Verify(block)
}

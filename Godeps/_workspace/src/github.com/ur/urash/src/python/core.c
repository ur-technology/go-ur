...
#define MIX_WORDS (URKASH_MIX_BYTES/4)
...
mkcache_bytes(PyObject *self, PyObject *args) {
    ...
    urash_light_t L = urash_light_new(block_number);
...
calc_dataset_bytes(PyObject *self, PyObject *args) {
    ...
    if (cache_size % URKASH_HASH_BYTES != 0) {
        char error_message[1024];
        sprintf(error_message, "The size of the cache must be a multiple of %i bytes (was %i)", URKASH_HASH_BYTES, cache_size);
        PyErr_SetString(PyExc_ValueError, error_message);
        return 0;
    }
    ...
    urash_compute_full_data(mem, &params, &cache);
...
hashimoto_light(PyObject *self, PyObject *args) {
    ...
    struct urash_return_value out = urash_light_compute(s, *h, nonce);
...
hashimoto_full(PyObject *self, PyObject *args) {
    ...
    urash_return_value out;
    urash_params params;
    params.full_size = (size_t) full_size;
    urash_full(&out, (void *) full_bytes, &params, (urash_h256_t *) header, nonce);
    return Py_BuildValue("{" PY_CONST_STRING_FORMAT ":" PY_STRING_FORMAT ", " PY_CONST_STRING_FORMAT ":" PY_STRING_FORMAT "}",
                         "mix digest", &out.mix_hash, 32,
                         "result", &out.result, 32);
}
...
mine(PyObject *self, PyObject *args) {
    ...
    do {
        urash_full(&out, (void *) full_bytes, &params, (const urash_h256_t *) header, nonce++);
        // TODO: disagrees with the spec https://github.com/ur/wiki/wiki/Ethash#mining
    } while (!urash_check_difficulty(&out.result, (const urash_h256_t *) difficulty));
...
get_seedhash(PyObject *self, PyObject *args) {
    ...
    if (!PyArg_ParseTuple(args, "k", &block_number))
        return 0;
    if (block_number >= URKASH_EPOCH_LENGTH * 2048) {
        char error_message[1024];
        sprintf(error_message, "Block number must be less than %i (was %lu)", URKASH_EPOCH_LENGTH * 2048, block_number);
...
static PyMethodDef PyurashMethods[] =
        {
                ...
                /*{"hashimoto_full", hashimoto_full, MURK_VARARGS,
...
#if PY_MAJOR_VERSION >= 3
static struct PyModuleDef PyurashModule = {
...
PyMODINIT_FUNC PyInit_pyurash(void) {
    PyObject *module =  PyModule_Create(&PyurashModule);
    // Following Spec: https://github.com/ur/wiki/wiki/Ethash#definitions
    PyModule_AddIntConstant(module, "REVISION", (long) URKASH_REVISION);
    PyModule_AddIntConstant(module, "DATASET_BYTES_INIT", (long) URKASH_DATASET_BYTES_INIT);
    PyModule_AddIntConstant(module, "DATASET_BYTES_GROWTH", (long) URKASH_DATASET_BYTES_GROWTH);
    PyModule_AddIntConstant(module, "CACHE_BYTES_INIT", (long) URKASH_CACHE_BYTES_INIT);
    PyModule_AddIntConstant(module, "CACHE_BYTES_GROWTH", (long) URKASH_CACHE_BYTES_GROWTH);
    PyModule_AddIntConstant(module, "EPOCH_LENGTH", (long) URKASH_EPOCH_LENGTH);
    PyModule_AddIntConstant(module, "MIX_BYTES", (long) URKASH_MIX_BYTES);
    PyModule_AddIntConstant(module, "HASH_BYTES", (long) URKASH_HASH_BYTES);
    PyModule_AddIntConstant(module, "DATASET_PARENTS", (long) URKASH_DATASET_PARENTS);
    PyModule_AddIntConstant(module, "CACHE_ROUNDS", (long) URKASH_CACHE_ROUNDS);
    PyModule_AddIntConstant(module, "ACCESSES", (long) URKASH_ACCESSES);
    return module;
}
#else
PyMODINIT_FUNC
initpyurash(void) {
    PyObject *module = Py_InitModule("pyurash", PyurashMethods);
    // Following Spec: https://github.com/ur/wiki/wiki/Ethash#definitions
    PyModule_AddIntConstant(module, "REVISION", (long) URKASH_REVISION);
    PyModule_AddIntConstant(module, "DATASET_BYTES_INIT", (long) URKASH_DATASET_BYTES_INIT);
    PyModule_AddIntConstant(module, "DATASET_BYTES_GROWTH", (long) URKASH_DATASET_BYTES_GROWTH);

/*
  This file is part of cpp-ur.

  cpp-ur is free software: you can redistribute it and/or modify
  it under the terms of the GNU General Public License as published by
  the Free Software Foundation, either version 3 of the License, or
  (at your option) any later version.

  cpp-ur is distributed in the hope that it will be useful,
...
#include <liburash/util.h>
#ifdef OPENCL
#include <liburash-cl/urash_cl_miner.h>
#endif
#include <vector>
#include <algorithm>
...
static std::string bytesToHexString(urash_h256_t const *hash, unsigned size)
{
...
extern "C" int main(void)
{
...
#ifdef FULL
	{
		auto startTime = high_resolution_clock::now();
		urash_return_value hash;
		urash_full(&hash, full_mem, &params, &previous_hash, 0);
		auto time = std::chrono::duration_cast<std::chrono::milliseconds>(high_resolution_clock::now() - startTime).count();
		debugf("urash_full test: %uns\n", (unsigned)time);
	}
#endif
	...
	{
		urash_return_value hash;
		urash_light(&hash, &cache, &params, &previous_hash, i);
	...
	#ifdef OPENCL
	{
		struct search_hook : urash_cl_miner::search_hook
		{
			unsigned hash_count;
		...
		for (unsigned i = 0; i != hook.nonce_vec.size(); ++i)
		{
			...
			urash_return_value hash;
			urash_light(&hash, &cache, &params, &previous_hash, nonce);
	...
	#else
	{
		...
		for (int nonce = 0; nonce < trials; ++nonce)
		{
			urash_return_value hash;
			#ifdef FULL
				urash_full(&hash, full_mem, &params, &previous_hash, nonce);
			#else
				urash_light(&hash, &cache, &params, &previous_hash, nonce);
			#endif // FULL
		}
	...
	unsigned read_size = URKASH_ACCESSES * URKASH_MIX_BYTES;

...
*/
/** @file io.h
 * @author Lefteris Karapetsas <lefteris@urdev.com>
 * @date 2015
 */
#pragma once
#include <stdlib.h>
...
// Reference: https://github.com/ur/wiki/wiki/Ethash-DAG
#define DAG_MUTABLE_NAME_MAX_SIZE (6 + 10 + 1 + 16 + 1)
/// Possible return values of @see urash_io_prepare
enum urash_io_rc {
	URKASH_IO_FAIL = 0,           ///< There has been an IO failure
	URKASH_IO_MEMO_SIZE_MISMATCH, ///< DAG with revision/hash match, but file size was wrong.
	URKASH_IO_MEMO_MISMATCH,      ///< The DAG file did not exist or there was revision/hash mismatch
	URKASH_IO_MEMO_MATCH,         ///< DAG file existed and revision/hash matched. No need to do anything
};
...
/**
 * Logs a critical error in important parts of urash. Should mostly help
 * figure out what kind of problem (I/O, memory e.t.c.) causes a NULL
 * urash_full_t
 */
#ifdef URKASH_PRINT_CRITICAL_OUTPUT
#define URKASH_CRITICAL(...)							\
	do													\
	{													\
		printf("URKASH CRITICAL ERROR: "__VA_ARGS__);	\
		printf("\n");									\
		fflush(stdout);									\
	} while (0)
#else
...
/**
 * Prepares io for urash
 *
 * Create the DAG directory and the DAG file if they don't exist.
 *
 * @param[in] dirname        A null terminated c-string of the path of the urash
 ...
 *                           https://github.com/ur/wiki/wiki/Ethash-DAG
 ...
 *                           already exists
 * @return                   For possible return values @see enum urash_io_rc
 */
enum urash_io_rc urash_io_prepare(
	char const* dirname,
	urash_h256_t const seedhash,
	FILE** output_file,
	uint64_t file_size,
	bool force_create
);
 ...
 */
FILE* urash_fopen(char const* file_name, char const* mode);
 ...
 *                         error returns NULL
 */
char* urash_strncat(char* dest, size_t dest_size, char const* src, size_t count);
 ...
 *                       existed
 */
bool urash_mkdir(char const* dirname);
 ...
 */
bool urash_file_size(FILE* f, size_t* ret_size);
 ...
 *
 * The spec defining this directory is here: https://github.com/ur/wiki/wiki/Ethash-DAG
 *
 ...
 */
bool urash_get_default_dirname(char* strbuf, size_t buffsize);

static inline bool urash_io_mutable_name(
	uint32_t revision,

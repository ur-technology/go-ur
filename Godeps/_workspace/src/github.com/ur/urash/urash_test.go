...
func TestEthashConcurrentSearch(t *testing.T) {
	...
	for i := 0; i < nsearch; i++ {
		go func() {
			nonce, md := ur.Search(block, stop, 0)
			select {
...
func TestGetSeedHash(t *testing.T) {
	...
	// From python:
	// > from pyurash import get_seedhash
	// > get_seedhash(30000)

...
func NewDebugApi(xur *xur.XEth, ur *ur.UR, coder codec.Codec) *debugApi {
	return &debugApi{
		xur:     xur,
		ur: ur,
...
func (self *debugApi) DumpBlock(req *shared.Request) (interface{}, error) {
	...
	stateDb, err := state.New(block.Root(), self.ur.ChainDb())
	if err != nil {
		return nil, err
	}
...
func (self *debugApi) SetHead(req *shared.Request) (interface{}, error) {
	...
	self.ur.BlockChain().SetHead(uint64(args.BlockNumber))
...
func (self *debugApi) ProcessBlock(req *shared.Request) (interface{}, error) {
	...
	statedb, err := state.New(blockchain.GetBlock(block.ParentHash()).Root(), self.ur.ChainDb())
	if err != nil {
		return false, err
	}
...
func (self *debugApi) SeedHash(req *shared.Request) (interface{}, error) {
	...
	if hash, err := urash.GetSeedHash(uint64(args.BlockNumber)); err == nil {

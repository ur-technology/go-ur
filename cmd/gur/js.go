...
import (
	...
	re "github.com/ur/go-ur/jsre"
	"github.com/ur/go-ur/rpc"
...
func newLightweightJSRE(docRoot string, client comms.URClient, datadir string, interactive bool) *jsre {
	js := &jsre{ps1: "> "}
...
func newJSRE(ur *ur.UR, docRoot, corsDomain string, client comms.URClient, interactive bool, f xur.Frontend) *jsre {
	js := &jsre{ur: ur, ps1: "> "}
	...
	js.xur = xur.New(ur, f)
	js.wait = js.xur.UpdateState()
	js.client = client
	if clt, ok := js.client.(*comms.InProcClient); ok {
		if offeredApis, err := api.ParseApiString(shared.AllApis, codec.JSON, js.xur, ur); err == nil {
	...
	if !liner.TerminalSupported() || !interactive {
		...
		lr := liner.NewLiner()
		js.withHistory(ur.DataDir, func(hist *os.File) { lr.ReadHistory(hist) })
		lr.SetCtrlCAborts(true)
		js.loadAutoCompletion()
		lr.SetWordCompleter(apiWordCompleter)
		lr.SetTabCompletionStyle(liner.TabPrints)
		js.prompter = lr
		js.atexit = func() {
			js.withHistory(ur.DataDir, func(hist *os.File) { hist.Truncate(0); lr.WriteHistory(hist) })
			lr.Close()
			close(js.wait)
		}
	}
...
// show summary of current gur instance
func (self *jsre) welcome() {
	self.re.Run(`
    (function () {
      console.log('instance: ' + web3.version.client);
      console.log(' datadir: ' + admin.datadir);
      console.log("coinbase: " + ur.coinbase);
      var ts = 1000 * ur.getBlock(ur.blockNumber).timestamp;
      console.log("at block: " + ur.blockNumber + " (" + new Date(ts) + ")");
    })();
  `)
	if modules, err := self.supportedApis(); err == nil {
...
func (js *jsre) apiBindings(f xur.Frontend) error {
	...
	apiImpl, err := api.ParseApiString(strings.Join(apiNames, ","), codec.JSON, js.xur, js.ur)
	if err != nil {
		utils.Fatalf("Unable to determine supported api's: %v", err)
	}

	jur := rpc.NewJur(api.Merge(apiImpl...), js.re, js.client, f)
	js.re.Set("jur", struct{}{})
	t, _ := js.re.Get("jur")
	jurObj := t.Object()

	jurObj.Set("send", jur.Send)
	jurObj.Set("sendAsync", jur.Send)
	...
	err = js.re.Compile("ur.js", re.Web3_JS)
	if err != nil {
	...
	_, err = js.re.Run("web3.setProvider(jur)")
	if err != nil {
		utils.Fatalf("Error setting web3 provider: %v", err)
	}

	// load only supported API's in javascript runtime
	shortcuts := "var ur = web3.ur; "
	...
	js.re.Run(`var GlobalRegistrar = ur.contract(` + registrar.GlobalRegistrarAbi + `);   registrar = GlobalRegistrar.at("` + registrar.GlobalRegistrarAddr + `");`)
	return nil
}
...
func (self *jsre) ConfirmTransaction(tx string) bool {
	if self.ur.NatSpec {
		notice := natspec.GetNotice(self.xur, tx, self.ur.HTTPClient())
		fmt.Println(notice)
...
func (self *jsre) UnlockAccount(addr []byte) bool {
	...
	// TODO: allow retry
	if err := self.ur.AccountManager().Unlock(common.BytesToAddress(addr), pass); err != nil {
		return false
	} else {

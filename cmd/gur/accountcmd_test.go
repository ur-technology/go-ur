...
func TestAccountListEmpty(t *testing.T) {
	gur := runGur(t, "account")
	gur.expectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	gur := runGur(t, "--datadir", datadir, "account")
	defer gur.expectExit()
...
func TestAccountNew(t *testing.T) {
	gur := runGur(t, "--lightkdf", "account", "new")
	defer gur.expectExit()
	gur.expect(`
...
`)
	gur.expectRegexp(`Address: \{[0-9a-f]{40}\}\n`)
}
...
// 	gur := runGur(t, "--lightkdf", "wallet", "import", "testdata/guswallet.json")
// 	defer gur.expectExit()
// 	gur.expect(`
// !! Unsupported terminal, password will be echoed.
...
// `)
//
// 	files, err := ioutil.ReadDir(filepath.Join(gur.Datadir, "keystore"))
// 	if len(files) != 1 {
...
`)
	...
	for _, m := range wantMessages {
		if strings.Index(gur.stderrText(), m) == -1 {
...
// https://github.com/ur-technology/go-ur/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	gur.setTemplateFunc("keypath", func(file string) string {

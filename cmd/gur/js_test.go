...
type testjurre struct {
	*jsre
	lastConfirm string
	client      *httpclient.HTTPClient
}

func (self *testjurre) UnlockAccount(acc []byte) bool {
	err := self.ur.AccountManager().Unlock(common.BytesToAddress(acc), "")
	if err != nil {
...
func (self *testjurre) ConfirmTransaction(tx string) bool {
	if self.ur.NatSpec {
		self.lastConfirm = natspec.GetNotice(self.xur, tx, self.client)
	}
	return true
}

func testJEthRE(t *testing.T) (string, *testjurre, *ur.UR) {
	return testREPL(t, nil)
}

func testREPL(t *testing.T, config func(*ur.Config)) (string, *testjurre, *ur.UR) {
	tmp, err := ioutil.TempDir("", "gur-test")
	if err != nil {
		t.Fatal(err)
	}
	...
	assetPath := filepath.Join(os.Getenv("GOPATH"), "src", "github.com", "ur", "go-ur", "cmd", "mist", "assets", "ext")
	client := comms.NewInProcClient(codec.JSON)
	tf := &testjurre{client: ur.HTTPClient()}
	repl := newJSRE(ur, assetPath, "", client, false, tf)
	tf.jsre = repl
	return tmp, tf, ur
}
...
func TestNodeInfo(t *testing.T) {
	...
	tmp, repl, ur := testJEthRE(t)
	if err := ur.Start(); err != nil {
		t.Fatalf("error starting ur: %v", err)
	}
	defer ur.Stop()
...
func TestAccounts(t *testing.T) {
	...
	checkEvalJSON(t, repl, `ur.accounts`, `["`+testAddress+`"]`)
	checkEvalJSON(t, repl, `ur.coinbase`, `"`+testAddress+`"`)
	...
	checkEvalJSON(t, repl, `ur.accounts`, `["`+testAddress+`","`+addr+`"]`)
...
func TestBlockChain(t *testing.T) {
	...
	val, err := repl.re.Run("JSON.stringify(debug.dumpBlock(ur.blockNumber))")
	if err != nil {
		t.Errorf("expected no error, got %v", err)
	}
	...
	// do the export
	extmp, err := ioutil.TempDir("", "gur-test-export")
	if err != nil {
		t.Fatal(err)
	}
	...
	checkEvalJSON(t, repl, `debug.dumpBlock(ur.blockNumber)`, beforeExport)
}
...
func TestMining(t *testing.T) {
	...
	defer ur.Stop()
	defer os.RemoveAll(tmp)
	checkEvalJSON(t, repl, `ur.mining`, `false`)
}
...
func TestRPC(t *testing.T) {
	...
	checkEvalJSON(t, repl, `admin.startRPC("127.0.0.1", 5004, "*", "web3,ur,net")`, `true`)
}
...
func TestCheckTestAccountBalance(t *testing.T) {
	...
	checkEvalJSON(t, repl, `ur.getBalance(primary)`, `"`+testBalance+`"`)
}
...
func TestSignature(t *testing.T) {
	...
	val, err := repl.re.Run(`ur.sign("` + testAddress + `", "` + testHash + `")`)
...
func TestContract(t *testing.T) {
	t.Skip("contract testing is implemented with mining in urash test mode. This takes about 7seconds to run. Unskip and run on demand")
	coinbase := common.HexToAddress(testAddress)
	tmp, repl, ur := testREPL(t, func(conf *ur.Config) {
		conf.URbase = coinbase
		conf.PowTest = true
	})
	if err := ur.Start(); err != nil {
		t.Errorf("error starting ur: %v", err)
	...
	* then set once more SetHashReg SetUrlHint
	...
	if checkEvalJSON(t, repl, `primary = ur.accounts[0]`, `"`+testAddress+`"`) != nil {
		return
	}
	...
	if err != nil {
		...
		if checkEvalJSON(t, repl, `contract = ur.compile.solidity(source).test`, string(contractInfo)) != nil {
			return
		}
	}
	...
	if checkEvalJSON(
		t, repl,
		`contractaddress = ur.sendTransaction({from: primary, data: contract.code})`,
...
func pendingTransactions(repl *testjurre, t *testing.T) (txc int64, err error) {
	txs := repl.ur.TxPool().GetTransactions()
	return int64(len(txs)), nil
}

func processTxs(repl *testjurre, t *testing.T, expTxc int) bool {
	var txc int64
	var err error
	for i := 0; i < 50; i++ {
...
func checkEvalJSON(t *testing.T, re *testjurre, expr, want string) error {

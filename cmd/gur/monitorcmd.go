...
func monitor(ctx *cli.Context) {
	...
	// Attach to an UR node over IPC or RPC
	...
	xur := rpc.NewXur(client)

	// Retrieve all the available metrics and resolve the user pattens
	metrics, err := retrieveMetrics(xur)
	if err != nil {
	...
	refreshCharts(xur, monitored, data, units, charts, ctx, footer)
	termui.Body.Align()
	...
	for {
		...
		case <-refresh:
			if refreshCharts(xur, monitored, data, units, charts, ctx, footer) {
				termui.Body.Align()
			}
			termui.Render(termui.Body)
		}
...
// retrieveMetrics contacts the attached gur node and retrieves the entire set
// of collected system metrics.
func retrieveMetrics(xur *rpc.Xur) (map[string]interface{}, error) {
	return xur.Call("debug_metrics", []interface{}{true})
}
...
func refreshCharts(xur *rpc.Xur, metrics []string, data [][]float64, units []int, charts []*termui.LineChart, ctx *cli.Context, footer *termui.Par) (realign bool) {
	values, err := retrieveMetrics(xur)

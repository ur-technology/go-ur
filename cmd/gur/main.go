...
// gur is the official command-line client for UR.
package main
...
func init() {
	...
	app = utils.NewApp(Version, "the go-ur command line interface")
	app.Action = run
...
`,
		...
		monitorCommand,
		{
			Action: makedag,
			Name:   "makedag",
			Usage:  "generate urash dag (for testing)",
			Description: `
The makedag command generates an urash DAG in /tmp/dag.
...
`,
		...
		{
			Name:  "wallet",
			Usage: "ur presale wallet",
			Subcommands: []cli.Command{
				{
					Action: importWallet,
					Name:   "import",
					Usage:  "import ur presale wallet",
				},
			},
			Description: `
...
will prompt for your password and imports your ur presale account.
...
between ur nodes by simply copying.
...
For non-interactive use the passphrase can be specified with the --password flag:
    ...
    ur --password <passwordfile> account new
...
For non-interactive use the passphrase can be specified with the --password flag:
    ...
    ur --password <passwordfile> account update <address>
...
For non-interactive use the passphrase can be specified with the -password flag:
    ...
    ur --password <passwordfile> account import <keyfile>

Note:
As you can directly copy your encrypted accounts to another ur instance,
...
See https://github.com/ur/go-ur/wiki/Javascipt-Console
`},
		{
			Action: attach,
			Name:   "attach",
...
which exposes a node admin interface as well as the Ðapp JavaScript API.
See https://github.com/ur/go-ur/wiki/Javascipt-Console.
This command allows to open a console on a running gur node.
`,
		},
		{
			Action: execJSFiles,
...
JavaScript API. See https://github.com/ur/go-ur/wiki/Javascipt-Console
`,
		},
	}
	app.Flags = []cli.Flag{
...
func blockRecovery(ctx *cli.Context) {
	...
	cfg := utils.MakeEthConfig(ClientIdentifier, nodeNameVersion, ctx)
	blockDb, err := urdb.NewLDBDatabase(filepath.Join(cfg.DataDir, "blockchain"), cfg.DatabaseCache)
	if err != nil {
...
func startEth(ctx *cli.Context, ur *ur.UR) {
	// Start UR itself
	utils.StartUR(ur)

	am := ur.AccountManager()
	...
	if !ctx.GlobalBool(utils.IPCDisabledFlag.Name) {
		if err := utils.StartIPC(ur, ctx); err != nil {
			utils.Fatalf("Error string IPC: %v", err)
		}
	}
	if ctx.GlobalBool(utils.RPCEnabledFlag.Name) {
		if err := utils.StartRPC(ur, ctx); err != nil {
...
func makedag(ctx *cli.Context) {
	args := ctx.Args()
	wrongArgs := func() {
		utils.Fatalf(`Usage: gur makedag <block number> <outputdir>`)
	}
	switch {
	case len(args) == 2:
...
func gpuinfo(ctx *cli.Context) {
	ur.PrintOpenCLDevices()
}

func gpubench(ctx *cli.Context) {
	args := ctx.Args()
	wrongArgs := func() {
		utils.Fatalf(`Usage: gur gpubench <gpu number>`)
	}
	switch {
	case len(args) == 1:
...
func version(c *cli.Context) {
	...
	fmt.Println("Protocol Versions:", ur.ProtocolVersions)
